...
Cleans the build folder recursively from all files. For AppVeyor-based builds also tries to determine the current build version by calling the `Get-SemanticVersion` function, and calls the `appveyor UpdateBuild` command if it's not null.
